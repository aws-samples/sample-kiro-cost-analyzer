...
- [x] **6.6*** Align the `boto3` pin across the three deploy manifests so `requirements-dev.txt` can return to pure composition. Done: `backend/requirements.txt` and `etl/requirements.txt` moved from `boto3==1.43.4` to `==1.43.92`, the current release, which clears every agent floor (`bedrock-agentcore` `>=1.43.31`, `bedrock-agentcore-starter-toolkit` `>=1.42.1`, `strands-agents` `>=1.26.0,<2`) and the `moto` bounds (`botocore>=1.20.88` excluding 1.35.45/46, outside this range). `requirements-dev.txt` now references all three manifests and restates nothing, closing the fidelity gap where the suite ran a different `boto3` than the Lambdas deploy with. Verified in a clean Python 3.13 environment: `pip install -r requirements-dev.txt` exits 0 with no conflict, and the suite passes 1007/1007. The constraint is now inverted — dropping the pin below `1.43.31` reintroduces `ResolutionImpossible`, so both the manifest header and this entry record it.
