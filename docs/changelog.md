...
Every Holmes CSR scan in this change set flagged `template.yaml`'s `ValidateSourceBucket` statement (`s3:ListBucket` on `Resource: "arn:aws:s3:::*"`), needed because `PUT /api/config/bucket` validates an admin-supplied, potentially cross-account bucket name via `HeadBucket` before persisting it — `HeadBucket` has no narrower IAM action, and the bucket name is arbitrary request-time input, not a deploy-time value that could be pinned to a naming convention.

Decision: rather than continuing to carry this as a documented accepted trade-off, the plan is to **remove the feature** — the Settings page's bucket-reconfiguration field, `PUT /api/config/bucket`, `handle_put_config_bucket`, and the `ValidateSourceBucket` IAM statement. Changing the source bucket after initial deploy is expected to be rare in practice; a redeploy with a new `SourceBucketName` parameter is an acceptable path for the few who need it. Not implemented in this change set — tracked as a `TODO` comment in `template.yaml` next to the affected statement and in `docs/security.md`.
