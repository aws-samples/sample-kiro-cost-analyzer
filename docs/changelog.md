...
- **Actions on a supported runtime** — `actions/checkout@v7`, `actions/setup-python@v7` and `actions/setup-node@v7`, all declaring `using: node24`. The versions the workflow first shipped with target Node 20, which GitHub deprecated: every run raised *"Node.js 20 is deprecated … being forced to run on Node.js 24"* in both jobs. Release notes for the skipped majors were checked first; the only behavioural change that could have mattered — `setup-node` v5 auto-caching when `package.json` declares a `packageManager` field — does not apply, since `frontend/package.json` has no such field and the explicit `cache: npm` still governs. `release.yml` and `publish-release.yml` were bumped to `checkout@v7` too, and `pr-title.yml`'s third-party `amannn/action-semantic-pull-request` from v5 to v6 — it was the last `node20` holdout, and GitHub's warning never named it because the message only enumerates `actions/*`. All four workflow files now run on `node24`. The two release workflows `git push` after checkout, so `persist-credentials` was confirmed to still default to `true` in v5–v7 first — neither is reachable from a pull request, so that verification had to come from reading each version's `action.yml` rather than from a green run.
...
### Fix — the five defects the new CI gates revealed

Running the suites for the first time exposed five defects that predated the workflow. All are fixed here, so both checks are green: `backend` 1007 passing (was 997 passing / 5 failing), `frontend` lint, test and build all exit 0 (lint was 42 errors, test did not run at all).

- **4 clock-dependent ETL history tests, red on `main` since 2026-08-25.** `backend/handlers/etl_executions_handler.py` derived its window from real wall-clock time while `tests/test_etl_executions_handler.py` hardcoded `_NOW = 2026-08-20` and never froze the clock, so once real time passed `_NOW + 5 days` every fixture execution fell outside the cutoff. `handle_etl_executions` now accepts an injectable `now` (resolved through `_resolve_now`), matching the dependency-injection style of its existing `sfn_client`/`dynamodb_resource` parameters; `backend/handler.py` is unchanged. A fifth test, `TestEnglishOnlyResponse`, had been passing **vacuously** — it asserted over an empty list — and is now meaningful. New `TestClockInjection` locks the behaviour.
- **A test that could only pass on a developer machine.** `test_missing_required_env_var_raises` clears `os.environ` and then calls `get_config()`, which built its SSM client *before* reading `os.environ["SSM_BUCKET_NAME"]` — so a misconfigured deployment surfaced `NoRegionError` from the client constructor instead of the `KeyError` the function documents, and the test depended on `~/.aws/config` for a region. `etl/config.py` now reads the required env paths first, which fixes the contract and avoids building a client that is never used. The test itself was not touched.
- **The frontend `test` script had never run.** It was `NODE_OPTIONS=--no-webstorage vitest --run`, and `--no-webstorage` exists in no Node release (verified on v22.23.2 and v24.20.0; the only flag in that family is the opt-in `--experimental-webstorage`), so `npm run test` exited 9 everywhere — while `CONTRIBUTING.md` documented it as a local gate. The flag is removed; `src/test/setup.ts` already carries a working localStorage fallback for the problem it was aimed at, so no behaviour changed.
- **4 frontend test failures — stale tests, not translation drift.** Three in `localeSwitchIntegration.test.tsx` predated both `DashboardPage` and `SettingsPage` moving their content into Cloudscape `Tabs`, which mounts only the active panel: the usage table and the Identity Store role ARN field were simply not in the DOM. They now activate the tab they need. Fixing them uncovered three more faults in that file: the fetch mock matched `/api/usage` before `/api/usage/account`, so the account call received the users payload; renders were never unmounted, so `screen` queries could resolve against an earlier test's tree (which is why the file passed in isolation and failed as a suite); and `SettingsPage`'s *uncontrolled* Tabs remounts on a locale change and silently resets to the first tab. The fourth failure was a `ptBrSnapshots` snapshot older than v3.8 — it still expected "Dispare o ETL" and had no execution-history table.
- **42 lint errors → 0.** Eight fixed in code: five `no-explicit-any` casts (dynamic i18n keys now narrow to `TranslationKey`, as `gitProviders.ts` already did; the mixed chart series is properly typed; `UsageTable`'s colour map is typed as `BoxProps['color']`) and three `no-misleading-character-class` reports (the emoji-stripping regex is hoisted to a module constant and rewritten as an alternation). The `UsageTable` change also fixes a latent bug the `as any` was hiding: the map's values were `color-text-status-*`, which is not a valid `BoxProps['color']`, so those colours never applied. The remaining 34 belong to `react-hooks/set-state-in-effect` (20) and `react-refresh/only-export-components` (14) — two rules that arrived through caret-ranged plugin upgrades and flag established patterns rather than new mistakes. They are set to `warn` in `frontend/eslint.config.js` with the reasoning recorded inline, stay visible in lint output, and should be promoted back to `error` as each cleanup lands.
...
- **Cross-region KMS wildcard (Medium, 3 sites)** — `KMSForGitTokens` (`BackendFunction`), `KMSDecryptGitTokens` (`CorrelationWorkerFunction`), and `KMSDecryptForSSM` (`AgentCoreAppPermissionsPolicy`) in `template.yaml` all granted `kms:Decrypt` on `arn:aws:kms:*:${AWS::AccountId}:key/*` — a region wildcard broader than needed for a single-region deployment. Scoped all three to `arn:aws:kms:${AWS::Region}:${AWS::AccountId}:key/*`.
- **Path traversal risk (High)** — `backend/handlers/prompts_handler.py`'s `handle_get_prompt_detail` interpolated the URL-path `requestId` directly into an S3 key (`prompts-content/{request_id}.json`) with only an implicit guard (a prior DynamoDB lookup that happens to only return existing IDs). Added an explicit `_REQUEST_ID_PATTERN` (`^[A-Za-z0-9_-]+$`) check that rejects `../`-style input with a 400 before the DynamoDB lookup, as defense-in-depth independent of that lookup.
- **`exc_info=True` in ETL user-name resolution (Medium, 5 sites)** — `etl/utils/name_resolver.py` and `etl/user_name_resolver.py` logged full tracebacks on Identity Store/DynamoDB failures via `exc_info=True`, which can echo table ARNs or Identity Store API response detail. Switched all five call sites to `exc_info=False`, matching the `errorType`-only pattern already used elsewhere in the ETL pipeline.
- **Unfiltered API error response text (Medium)** — `frontend/src/api/client.ts` fell back to the raw, unparsed response body (`apiMessage = text`) when an error response wasn't valid JSON or lacked a `message` field. Changed the fallback to a generic `Erro na requisição ({status})` message; only the API's own `message` field is ever surfaced.
- **Missing redaction in Lambda-layer-fallback loggers (Medium, 2 sites)** — `backend/utils/logging.py` and `etl/utils/logging.py` (used when the shared Lambda layer isn't importable, e.g. local test runs) were plain copies of `StructuredLogger` without the `_redact_sensitive()` filter present in `layers/shared/shared/structured_logger.py`. Added the same redaction logic to both, plus a docstring note to keep the three copies in sync.
- **Orphaned, out-of-sync dependency file (informational)** — `agent/app/GitCorrelationAgent/pyproject.toml` still pinned `strands-agents==1.38.0` (vulnerable to CVE-2026-18394) while the actually-used `requirements.txt` in the same directory had already been bumped to `1.50.2`. Confirmed via the `bedrock-agentcore-starter-toolkit` source that `agentcore deploy` reads `requirements.txt` first and only falls back to `pyproject.toml` if it's absent — this file was never read by the deploy path. Removed it rather than re-syncing a file with no consumer; updated the two comments that referenced it (`.kiro/specs/agent-git-correlation/design.md`, `ssm_token.py`).
- **Scan hygiene** — added `.holmesignore` mirroring `.gitignore` (plus `frontend/.env.production` explicitly) after a scan uploaded a locally-generated `.env.production` containing this developer's live API Gateway URL and Cognito IDs; that file is gitignored and never committed, so it should never have reached a scan upload.
- **Verified false positives, no code change** — `template.yaml`'s CloudFront TLS/S3 access-logging/SSL-enforcement findings and the `ssm_token.py` credential-disclosure finding remain flagged by Holmes on every scan despite being suppressed in-line (`checkov: skip:`, `guard: SuppressedRules:`, `# nosemgrep`) with a documented rationale; these are scanner-side re-flags of an already-mitigated pattern, not new gaps.
- **Tests** — `tests/test_prompts_handler.py` gains `test_returns_400_for_path_traversal_request_id` and `test_returns_400_for_request_id_with_slash`; `test_uses_default_data_bucket_when_env_not_set` rewritten to assert the fail-loud 500 behavior (the hardcoded bucket-name fallback it tested was already removed in the entry below). Full suite: 807 Python tests passing.
